            ...
            <form method="post" action=".?route=taskList&action=edit">
                <?php
                    if(isset($taskToUpdate))
                    {
                        ...
                        echo '<input type="time" class="form-control" name="description" placeholder="Date de travail" value="'.$taskToUpdate->getWorkDuration().'">';
                        echo '</div>';
                        echo '</div>';
                        echo '<button type="submit" class="btn btn-primary" value="'.$taskToUpdate->getId().'" name="idTask">'.'Modifier'.'</button>';
                        // echo '<label>'.'Nom de la tâche'.'</label>';
                        // echo '<input type="text" class="form-control mb-2 col-md-4" name="title" placeholder="title" value="'.$taskToUpdate->getTitle().'" readonly>';
                        // echo '<label>'.'Description'.'</label>';
                        // echo '<input type="text" class="form-control mb-2 col-md-4" name="description" placeholder="description" value="'.$taskToUpdate->getDescription().'">';
                        // echo '<label>'.'Localisation'.'</label>';
                        // echo '<input type="text" class="form-control mb-2 col-md-1" name="location" placeholder="location" value="'.$taskToUpdate->getLocation().'">';
                        // echo '<button type="submit" class="btn btn-primary btn-lg" value="'.$taskToUpdate->getId().'" name="idTask">'.'Modifier'.'</button>';
                    }
                ?>
            </form>
        </div>
